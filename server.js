...
io.on('connection', async (socket) => {
  ...
  socket.on('join', async ({ username, preferredWeapon, authToken }) => {
    ...
    const playerState = {
      ...
      health: 100,
      maxHealth: 100,
      currentWeapon: preferredWeapon || playerDoc.currentWeapon || null,
  ...
  socket.on('command', async ({ command, target, value }) => {
    ...
    // Track 8: clan/friend commands (reliable text entry via chat command path).
    const cmdHead = (command || '').split(' ')[0];
    if (cmdHead === 'clan' || cmdHead === 'friend') {
      const parts = [command, target, value].filter(Boolean).join(' ').trim().split(/\s+/);
      const Clan = require('./models/Clan');
      const Friendship = require('./models/Friendship');
      try {
        if (parts[0] === 'clan') {
          ...
          if (sub === 'create') {
            const name = parts[2];
            const tag = (parts[3] || '').toUpperCase().slice(0, 5);
            if (!name || !tag) { socket.emit('clanInfo', { message: 'Usage: /clan create <name> <tag>' }); return; }
            if (await Clan.findOne({ name })) { socket.emit('clanError', { message: 'Clan name taken' }); return; }
            const clan = await Clan.create({ name, tag, owner: player.username, members: [{ username: player.username, rank: 'leader' }] });
            await Player.updateOne({ username: player.username }, { $set: { clanId: name } });
            player.clanId = name;
          ...
          if (parts[1] === 'add' && parts[2]) {
            const tgt = parts[2].toLowerCase();
            if (tgt === player.username) { socket.emit('friendError', { message: 'Cannot add yourself' }); return; }
            if (!(await Player.findOne({ username: tgt }))) { socket.emit('friendError', { message: 'User not found' }); return; }
            await Friendship.updateOne({ requester: player.username, recipient: tgt }, { $setOnInsert: { status: 'accepted' } }, { upsert: true });
            socket.emit('friendInfo', { message: `Added ${tgt} as friend` });
          } else {
            socket.emit('friendInfo', { message: 'Usage: /friend add <username>' });
          }
        }
